...
## Last Change: **`/banquet`'s header was visibly broken above 1024px — logo squished to a 5px sliver, "Reserve Banquet Hall" pushed off-screen with page-level horizontal scroll.** Root cause: the logo `<Image>`/`<Link>` and service-switcher pills had no `shrink-0`, so when `BanquetHeader.tsx`'s (uncommitted, pre-existing) longer nav labels ("Halls & Capacity", "Packages & Enquiry") pushed total header content past the available width, flexbox shrank the logo first — and since its height was pinned by a Tailwind class while width was `auto`, it rendered non-uniformly squashed instead of shrinking proportionally. Fixed: added `shrink-0` to the logo `Link`/`Image`/pills-`nav` in all three headers (`RestaurantHeader.tsx`, `BanquetHeader.tsx`, `CateringHeader.tsx`, defensively, since they share the exact same structure), shortened the banquet nav labels back to one word each (hrefs unchanged), and moved the banquet phone number's breakpoint from `xl:` (1280px) to `2xl:` (1536px) since that's what was tipping the row over at common laptop widths. Verified at 1280/1440/1920px: no squish, no overflow, matches Restaurant/Catering.
## Previous Change (v2.11.2)
