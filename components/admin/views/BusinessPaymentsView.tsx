...
import { Plus, Search, CheckCircle2, XCircle, Clock, ChevronDown, X } from "lucide-react";
import { useBusinessAdminStore, PaymentStatus } from "@/store/useBusinessAdminStore";

export function BusinessPaymentsView() {
  const { payments, updatePaymentStatus, addPayment, orders } = useBusinessAdminStore();
  ...
  const [amount, setAmount] = useState(1290);
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between rounded-2xl border border-[#E5E7EB] bg-white p-5 shadow-xs">
        <div>
          ...
          <p className="text-xs text-[#6B7280]">
            Audit transactions. <strong className="text-rose-600 font-bold">{failedCount} Failed Payments</strong> currently recorded.
          </p>
        </div>
            ...
            <tbody className="divide-y divide-[#E5E7EB]">
              {filteredPayments.map((p) => (
                <tr key={p.id} className="hover:bg-[#F9FAFB] transition-colors">
                  ...
                  <td className="px-4 py-3.5 text-right text-[#6B7280]">{p.time}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-xs p-4">
          <div className="w-full max-w-md rounded-2xl border border-[#E5E7EB] bg-white p-6 shadow-xl space-y-4">
            <div className="flex items-center justify-between border-b border-[#E5E7EB] pb-3">
              <h3 className="text-base font-bold text-[#111827]">Record Payment</h3>
