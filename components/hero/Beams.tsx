...
function extendMaterial(
  BaseMaterial: new (params?: THREE.MeshStandardMaterialParameters) => THREE.Material,
...
export function Beams({
  ...
  vec3 getNormal(vec3 pos) {
    ...
    gl_FragColor.rgb -= randomNoise / 15. * uNoiseIntensity;`,
        ...
        material: { fog: true },
        uniforms: {
          diffuse: new THREE.Color(...hexToNormalizedRGB("#f1f5f9")),
...
const MergedPlanes = forwardRef<THREE.Mesh, MergedPlanesProps>(({ material, width, count, height }, ref) => {
  ...
  useFrame((_, delta) => {
    ...
    if (mat?.uniforms?.time) mat.uniforms.time.value += 0.1 * delta;
  });
...
function DirLight({ position, color }: { position: [number, number, number]; color: string }) {
  ...
  return <directionalLight ref={dir} color={color} intensity={3.5} position={position} />;
}
