...
export function LightPillar({
  ...
  useEffect(() => {
    ...
    const isLowEndDevice =
      isMobile || (typeof navigator !== "undefined" && navigator.hardwareConcurrency && navigator.hardwareConcurrency <= 4);
    ...
    try {
      // Silence internal THREE.js console.error spams when browser GPU context creation fails
      console.error = () => {};
    ...
    const fragmentShader = `
      ...
      vec3 myTanh(vec3 x) {
        vec3 clampedX = clamp(x, -10.0, 10.0);
        vec3 ex = exp(clampedX);
        vec3 emx = exp(-clampedX);
      ...
      void main() {
        ...
        col = myTanh(col * uGlowAmount / widthNorm);
  ...
  }, [webGLSupported, quality, interactive, glowAmount, intensity, noiseIntensity, pillarHeight, pillarRotation, pillarWidth, rotationSpeed, topColor, bottomColor]);
